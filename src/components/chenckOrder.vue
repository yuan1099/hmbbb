  ...
  data: function() {
    ...
    return {
        ...
        rules: {
            // 对应上面的name字段
         accept_name: [
            //   规则1：required  必填，message  提示信息，trigger  触发时机
            { required: true, message: '请输入活动名称', trigger: 'blur' },
            //   规则2：min 最小值   max 最大值，message  提示信息，trigger  触发时机
            { min: 2, max: 10, message: '长度在 3 到 10 个字符', trigger: 'change' }
          ],
          address:[
               { required: true, message: '请输入详细地址', trigger: 'blur' },

                { min: 2,  message: '最少2个字符', trigger: 'change' }
          ],
           mobile: [
          ...
          email: [
            ...
            post_code: [
            { validator: validatepost_code, trigger: 'change' }
          ],
        }
    };
  },
